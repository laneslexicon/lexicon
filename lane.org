...
* open root in a free floating independent window that can be printed etc
* Options

  + click/double click in tree
  + new root opens in new tab, appends to existing or replaces existing
  + arabic font in tab title
  + history serialize for how long
  + text width for graphicsitem
