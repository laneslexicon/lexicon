...
if ($help) {
  print STDERR "Usage:\n";
  print STDERR "perl jdata.pl <options>\n";
  print STDERR "             --map <map file name>       Full path to the map file. Required.\n";
  print STDERR "             --out <output file name>    Name of the ouput file. If not supplied the input file name is used\n";
  print STDERR "                                         with the suffix replaced by 'js' (if present) or with '.js' appended (if not present).\n";
  print STDERR "             --outdir <output directory> Where the output file is created. Optional. If not supplied the systems tmp directory is used.\n";
  print STDERR "             --unicode <file name>       The full path to UnicodeData.txt. If not supplied assumes ../data/UnicodeData.txt\n";
  print STDERR "             --overwrite                 Overwrite an existing output file\n";
  exit 1;
}
if (! $outfile ) {
  my @suffixes;
